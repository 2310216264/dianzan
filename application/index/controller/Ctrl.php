...
class Ctrl extends Base
{
    ...
    public function lixibao()
    {
        ...
        //列表
        $lixibao = Db::name('xy_lixibao_list')->where('status',1)->field('id,name,bili,day,min_num')->order('day asc')->select();
    ...
    public function lixibao_ru()
    {
        ...
        if(request()->isPost()){
            ...
            $res = Db::name('xy_lixibao')->insert([
                ...
                'yuji_num'    => $yuji,
                'type'        => 1,
                'shouxu'      =>$lixibao['shouxu']*$price,
                'status'      => 0,
                'uptime'      =>strtotime(date('Y-m-d'))
            ]);
    ...
    public function lixibao_chu()
    {
        ...
        if(request()->isPost()){
            ...
            $oldprice = $price;
            $shouxu = $lxbParam['shouxu'] ?$lxbParam['shouxu']: 0;
            if($issy==1){
                $shouxu=0;
                $price = $price +$lixibao['real_num'];
            }else{
