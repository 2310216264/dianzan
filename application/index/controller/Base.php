...
//-        if($uinfo['deal_status']==3) return ['code'=>1,'info'=>'该账户存在未完成订单，无法继续任务！'];
        if($uinfo['balance']<config('deal_min_balance')) return ['code'=>1,'info'=>'余额低于'.config('deal_min_balance').'，无法继续交易'];
//        $count = db('xy_convey')->where('addtime','between',[strtotime(date('Y-m-d')),time()])->where('uid',session('user_id'))->count('id');//统计当天完成交易的订单
