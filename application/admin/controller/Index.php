...
class Index extends Controller
{
    ...
    public function main()
    {
        ...
        //订单总额
        $this->order_sum = db('xy_convey')->sum('commission');
        $this->today_order_sum = db('xy_convey')->where('addtime','between',[strtotime(date('Y-m-d')),time()])->sum('	commission');
        $this->yes_order_sum = db('xy_convey')->where('addtime','between',[$yes1,$yes2])->sum('	commission');
    ...
    public function Update($isreturn)
    {
        ...
        $isHtml = $isreturn?0:1;
        $con = 'A5源码（https://www.a5yuanma.cn）精品资源分享网站 ';
